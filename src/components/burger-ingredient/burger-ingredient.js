...
function BurgerIngredient({ item, handleIngredientClick }) {
  ...
  return (
    ...
    >
        {(cartIngredients.includes(item) || cartBun === item) && <Counter count={item.type !== "bun" ? cartIngredients.reduce((stack, value) => {if (value === item) stack += 1; return stack;}, 0): 1} size="default" extraClass="m-1" />}
        <img src={item.image} alt={`изображение ингредиента ${item.name}`} className={`${burgerIngredient.img} mb-2`}/>
